...
// node_modules/leo-logger/index.js
var require_leo_logger = __commonJS({
  "node_modules/leo-logger/index.js"(exports2, module2) {
    "use strict";
    var merge = require_lodash().merge;
    var settings = [];
    ...
    module2.exports = function(namespace) {
      ...
      function logLazy(type, method, fn) {
        ...
        if (config && config.trace) {
          ...
          mySettings.config.printTimestamp && type !== "time" && args.unshift(`[${(/* @__PURE__ */ new Date()).toUTCString()}]`);
      ...
      return {
        sub: (n) => {
          if (!(n in subLogger)) {
            subLogger[n] = module2.exports(namespace + "." + n);
          }
          return subLogger[n];
        },
        ...
        configure: function(s, config) {
          ...
          Object.keys(namespaceSettings).forEach((namespace2) => generateSettings(namespace2, true));
        }
      };
    };
    Object.assign(module2.exports, module2.exports("global"));
    module2.exports.configure(/.*/, {
      error: true
    });
    if (process.env.LEO_LOGGER) {
      if (process.env.LEO_LOGGER == "true") {
        module2.exports.configure(true);
      } else {
        let loggers = process.env.LEO_LOGGER.split(";");
        loggers.forEach((logger2) => {
          let parts = logger2.split("/");
          let regex;
          let config;
          let lookup = {
            a: "all",
          ...
          module2.exports.configure(regex, config);
        });
      }
    }
  }
...
// api/setup-grist.ts
var setup_grist_exports = {};
__export(setup_grist_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(setup_grist_exports);

// lib/grist-functions.ts
var import_grist_api = __toESM(require_dist());
var logger = __toESM(require_leo_logger());
var GRIST_URL = process.env.GRIST_URL || "http://localhost:8484/api";
var LEASE_END_ORG_NAME = process.env.LEASE_END_ORG_NAME || "Lease End - Finance";
var INCOME_STATEMENT_WORKSPACE_NAME = process.env.INCOME_STATEMENT_WORKSPACE_NAME || "Income Statement";
var BALANCE_SHEET_WORKSPACE_NAME = process.env.BALANCE_SHEET_WORKSPACE_NAME || "Balance Sheet";
var CASH_FLOW_WORKSPACE_NAME = process.env.CASH_FLOW_WORKSPACE_NAME || "Cash Flow";
var HOME_WORKSPACE_NAME = "Home";
var INCOME_STATEMENT_DOC_NAME = "Income Statement";
var INCOME_STATEMENT_TABLE_NAME = "Income_Statement";
var initializeGristAPI = () => {
  return new import_grist_api.GristDocAPI({ server: GRIST_URL });
};
var getOrg = async (api) => {
...
var cleanupHomeWorkspace = async (api, org) => {
...
var getOrCreateWorkspace = async (api, org, workspaceName) => {
  try {
    ...
    logger.error("[Error getting workspace]", error2);
    throw error2;
  }
};
var createFinancialWorkspaces = async (api, org) => {
  try {
    const workspaces = {
    ...
    logger.error("[Error creating workspaces]", error2);
    throw error2;
  }
};
var getOrCreateDocument = async (api, workspace, docName) => {
  try {
    ...
    logger.error("[Error getting document]", error2);
    throw error2;
  }
};
var createIncomeStatementTable = async (api, doc) => {
  try {
    ...
    logger.error("[Error creating income statement table]", error2);
    throw error2;
  }
};
var addSampleIncomeData = async (api, tableName, doc) => {
  try {
    const sampleData = [
      {
        date: "2024-01-01",
    ...
    logger.error("[Error adding sample data]", error2);
    throw error2;
  }
};
...
lodash/lodash.js:
   ...
   *)
*/
//# sourceMappingURL=setup-grist.js.map
